    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-neu-text-light">
                Rutas de aprendizaje asignadas a tu ficha/grupo
            </p>
        </div>
        <div class="flex flex-wrap gap-2">
            ...
            <a href="<?= Router::url('/profile') ?>" class="neu-flat px-4 py-2 rounded-full text-sm">
...
<?php if ($ficha === ''): ?>
    <div class="neu-flat p-12 text-center">
        <div class="text-6xl mb-4"><i class="fas fa-id-card text-neu-text-light"></i></div>
        <h3 class="text-xl font-bold text-neu-text-main mb-2">Ficha no definida</h3>
        <p class="text-neu-text-light mb-6">Tu usuario no tiene ficha asignada. Pide al instructor que la configure o registrate con ficha.</p>
    </div>
<?php elseif (empty($routes)): ?>
    <div class="neu-flat p-12 text-center">
        <div class="text-6xl mb-4"><i class="fas fa-route text-neu-text-light"></i></div>
        <h3 class="text-xl font-bold text-neu-text-main mb-2">Sin rutas asignadas</h3>
        <p class="text-neu-text-light">No hay rutas activas para tu ficha (<strong><?= htmlspecialchars($ficha) ?></strong>).</p>
    </div>
<?php else: ?>
    <!-- Badge de ficha -->
    <div class="mb-4">
        <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-sena-green bg-opacity-10 text-sena-green text-sm font-semibold">
        ...
        <?php foreach ($routes as $r): ?>
            ...
            <div class="neu-flat transition-all hover:shadow-lg rounded-xl">
                ...
                <div class="p-5">
                    ...
                    <div class="mt-5">
                        <a
                            href="<?= Router::url('/routes/' . (int) ($r['id'] ?? 0)) ?>"
                            class="w-full neu-btn-primary px-4 py-2 rounded-full text-center text-sm block"
                        >
                            <i class="fas fa-play mr-1"></i> Ver Ruta
                        </a>
                    </div>
                </div>
            </div>
